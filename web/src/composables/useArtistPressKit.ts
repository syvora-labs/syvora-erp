...
export function useArtistPressKit() {
    ...
    async function uploadFile(
        ...
        // client-side, push the binary to Storage, then insert the metadata row with the final
        // storage_path already set. This avoids any placeholder value on the UNIQUE column
        // (previously 'pending'), which could collide between concurrent uploads.
        ...
        if (insertError) {
            // Storage upload succeeded but the metadata row was rejected — clean up the
            // orphan object so we don't leak bytes.
            await supabase.storage.from('press-kits').remove([storagePath]).catch(() => undefined)
            throw insertError
        }
