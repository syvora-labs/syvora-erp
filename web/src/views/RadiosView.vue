...
function openCreate() {
    ...
    form.value = { title: '', description: '', artists: [], release_date: '' }
    pendingFiles.value = []
    newFileLabel.value = ''
    error.value = ''
            ...
            <div v-else class="radios-list">
                ...
                >
                    <div class="radio-body">
                        ...
                        <div v-if="radio.files.length" class="radio-files">
                            <span class="files-label">Files:</span>
                            <div v-for="f in radio.files" :key="f.id" class="file-chip">
                                <span class="file-chip-label">{{ f.label }}</span>
                                <span v-if="f.file_size" class="file-chip-size">{{ formatFileSize(f.file_size) }}</span>
                                <button class="file-chip-action" title="Download" @click="downloadFile(f)">&#8595;</button>
                                <button class="file-chip-action file-chip-delete" title="Delete" @click="handleDeleteFile(f)">&times;</button>
                            </div>
                        </div>
                        ...
                        <div class="radio-footer">
                            <div class="radio-actions">
                                <SyvoraButton v-if="radio.is_draft" size="sm" @click="handlePublish(radio)">
                                    Publish
                                </SyvoraButton>
